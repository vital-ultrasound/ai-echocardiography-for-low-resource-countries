...
void Worker_FourChDetection::doWork(ifind::Image::Pointer image){
    ...
    this->gstate = PyGILState_Ensure();
    {
        ...
        if (this->params.verbose){
            std::cout << "[VERBOSE][Worker_FourChDetection::doWork()]  Atention added"<<std::endl;
        ...
        if (this->params.verbose){
            std::cout << "[VERBOSE][Worker_FourChDetection::doWork()]  Unresize"<<std::endl;
        ...
        if (this->params.verbose){
            std::cout << "[VERBOSE][Worker_FourChDetection::doWork()]  Uncrop"<<std::endl;
    ...
    if (this->params.verbose){
        std::cout << "[VERBOSE][Worker_FourChDetection::doWork()]  imported image"<<std::endl;
    }
...
void Worker_FourChDetection::slot_lungZoneButtonToggled(int i, int s){
    //std::cout << "[Worker_FourChDetection::slot_predictionsButtonToggled] text is "<<button->text().toStdString()<< " with value "<< checked << std::endl;
    if (i > 0){
        QString lungZone("L");
        if (s==1){
            lungZone = "R";
...
void Worker_FourChDetection::slot_predictionsButtonToggled(QAbstractButton *button, bool checked){
    //std::cout << "[Worker_FourChDetection::slot_predictionsButtonToggled] text is "<<button->text().toStdString()<< " with value "<< checked << std::endl;
    if (checked){
        mutex_latestData.lock();
...
void Worker_FourChDetection::slot_confidencesButtonToggled(QAbstractButton *button, bool checked){
    //std::cout << "[Worker_FourChDetection::slot_confidencesButtonToggled] text is "<<button->text().toStdString()<< " with value "<< checked << std::endl;
    if (checked){
        mutex_latestData.lock();
...
void Worker_FourChDetection::slot_Reset(){
    // Reset the data
...
void Worker_FourChDetection::slot_Submit(){
...
QString Worker_FourChDetection::makeSummaryString(){
